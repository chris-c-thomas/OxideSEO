...
impl SeoRule for BrokenInternal {
    fn id(&self) -> &str { "links.broken_internal" }
    fn name(&self) -> &str { "Broken Internal Link" }
    fn category(&self) -> RuleCategory { RuleCategory::Links }
...
impl SeoRule for RedirectChain {
    fn id(&self) -> &str { "links.redirect_chain" }
    fn name(&self) -> &str { "Redirect Chain" }
    fn category(&self) -> RuleCategory { RuleCategory::Links }
...
impl SeoRule for NofollowInternal {
    fn id(&self) -> &str { "links.nofollow_internal" }
    fn name(&self) -> &str { "Internal Nofollow Link" }
    fn category(&self) -> RuleCategory { RuleCategory::Links }
    fn default_severity(&self) -> Severity { Severity::Info }

    fn evaluate(&self, page: &ParsedPage, _ctx: &CrawlContext) -> Vec<Issue> {
        let nofollow_internals: Vec<_> = page.links.iter()
        ...
        if !nofollow_internals.is_empty() {
            vec![Issue {
                ...
                message: format!("{} internal link(s) have rel=\"nofollow\".", nofollow_internals.len()),
