 ...
 * between states (running, paused, completed, stopped, error). Updates
 * the crawl store and shows toast notifications for terminal states.
 *
...
export function useCrawlStateEvents() {
  ...
  useEffect(() => {
    ...
    const subscribe = async () => {
      unlisten = await listen<CrawlStateChange>("crawl://state", (event) => {
