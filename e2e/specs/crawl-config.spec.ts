...
test.describe("Crawl Config Form", () => {
  ...
  test("renders form with heading", async ({ page }) => {
    await expect(page.getByRole("heading", { name: "Configure Crawl" })).toBeVisible();
    await expect(
  ...
  test("submitting with empty URL shows validation error", async ({ page }) => {
...
test.describe("Crawl Config Advanced Sections", () => {
  ...
  test("Custom Headers section expands and collapses", async ({ page }) => {
    ...
    await trigger.click();
    await expect(page.getByText("Inject custom HTTP headers")).toBeVisible();

    // Collapse
    await trigger.click();
    await expect(page.getByText("Inject custom HTTP headers")).toBeHidden();
  });
  ...
  test("URL Include / Exclude Patterns section expands", async ({ page }) => {
    await page.getByRole("button", { name: /URL Include.*Exclude/i }).click();
    await expect(page.getByText("Regex patterns to control which URLs")).toBeVisible();
  });

  test("JavaScript Rendering section shows toggle", async ({ page }) => {
    await page.getByRole("button", { name: "JavaScript Rendering" }).click();
    await expect(page.getByText("Enable JS rendering for SPA detection")).toBeVisible();
  });
  ...
  test("Sitemap & External Links section has toggles", async ({ page }) => {
    await page.getByRole("button", { name: /Sitemap.*External/i }).click();
    await expect(page.getByText("Enable sitemap auto-discovery")).toBeVisible();
    await expect(page.getByText(/Check external links/i)).toBeVisible();
  });
});
